...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir='logs', comment='key points:{}  vote_layer_index:{}'.format(args.k_val, args.vote_idx))
    ...
    if not args.resume:
        last_epoch = 0
        os.system("mkdir saved_models_k{}_vote{}".format(args.k_val, args.vote_idx))
    else:
        ckpt_list = glob.glob("saved_models_k{}_vote{}/model_*".format(args.k_val, args.vote_idx))
        epoch_idx = []
        epoch_idx += [int(ckptname.replace("saved_models_k{}_vote{}/model_".format(args.k_val, args.vote_idx), '').replace('.pt', '')) for ckptname in ckpt_list]
        last_epoch = np.max(epoch_idx)
        model.load_state_dict(torch.load("saved_models_k{}_vote{}/model_{}.pt".format(args.k_val, args.vote_idx, last_epoch)))
        model.eval()
    ...
    for epoch in range(last_epoch, epoches):
        ...
        for class_idx in range(NUM_CLASSES):
            ...
            print(epoch, 'class_idx_{}: recall'.format(class_idx), np.mean(recalls_list[class_idx]), \
                    file = open("saved_models_k{}_vote{}/perf.log".format(args.k_val, args.vote_idx), 'a+'))
            ...
            print(epoch, 'class_idx_{}: mAP'.format(class_idx), np.mean(mAP_list[class_idx]), \
                    file = open("saved_models_k{}_vote{}/perf.log".format(args.k_val, args.vote_idx), 'a+'))
        # save model
        torch.save(model.state_dict(), "saved_models_k{}_vote{}/model_{}.pt".format(args.k_val, args.vote_idx, epoch))
